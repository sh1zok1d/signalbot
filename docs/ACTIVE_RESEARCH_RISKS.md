...
| RR-029 | HIGH | **MARKET-05 `NO_EVIDENCE` reinterpreted as ETH-useless, as validated divergence, as a reverse-sign effect, or as authorization to create MARKET-06 / a rescue child.** Canonical RESULT is `828c6916…` / `MARKET_05_NO_EVIDENCE`. | Keep the exact formulation spent. Do not rerun. Do not inspect protected OOS. Do not treat negative relative MAE as a reverse hypothesis. |
| RR-030 | HIGH | **Immediate further hypothesis generation on the same price/OI/cross-asset information set after several spent historical formulations with zero internal promotions.** Adaptive-search risk now dominates remaining nearby transforms. | Keep `SAME_INFORMATION_SET_HYPOTHESIS_GENERATION_PAUSED = true`. Do not create MARKET-06. Next phase is `INFORMATION_SET_EXPANSION`, not another nearby feature. |
| RR-031 | HIGH | **Forward collector `exchange_event_time` treated as `legal_available_at`, parsed records treated as raw evidence, smoke/early bytes used as scientific evidence, or early collected observations used to tune a later hypothesis.** MARKET-04 remains blocked on historical provenance, not mechanism failure. Collector implementation is frozen; authoritative collection has not started. | Keep `legal_available_at = local_received_at_utc` until a separate proof. Keep raw append-only evidence as root. Label smoke `INFRASTRUCTURE_SMOKE_TEST_ONLY`. Do not start authoritative collection except via `FORWARD_MARKET_OBSERVABILITY_V1_AUTHORITATIVE_COLLECTION_START`. Do not compute future return/MAE/drawdown/prediction/beta/classification during collection. Do not create M04-FWD from this freeze. Do not attempt a third historical proxy salvage. |
