...
| RR-031 | HIGH | **Forward collector `exchange_event_time` treated as `legal_available_at`, parsed records treated as raw evidence, or early collected observations used to tune a later hypothesis.** MARKET-04 remains blocked on historical provenance, not mechanism failure. | Keep `legal_available_at = local_received_at_utc` until a separate proof. Implement `FORWARD_MARKET_OBSERVABILITY_V1` as acquisition-only. Do not compute future return/MAE/drawdown/prediction/beta/classification during collection. Do not attempt a third historical proxy salvage. |
