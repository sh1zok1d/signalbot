...
| RR-026 | HIGH | **MARKET-03 public strategy treated as independently preregistered, or perpetual CORE used as the author's spot market.** The selected `EmaCross`/`EmaCrossFunding` parameters and BTC focus were chosen after benchmark/sweeps/cross-asset inspection. CORE is USD-M perpetual; the author trades spot. Funding `fundingTime` is not a proven `legal_available_at`. | Keep MARKET-03 at source-feasibility until required spot history and/or an explicit funding-availability assumption is acquired. Do not silently substitute CORE perp klines. Do not invent publication latency. |
