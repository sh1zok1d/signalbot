...
Production durability, aggregation, and result persistence are now
implemented. Canonical production execution requires a fresh process and a
tracked run identity bound to the exact execution commit. Local #114
reservation is superseded on this HEAD. Monte Carlo remains unarmed
(`production_monte_carlo_arm_authorized = false`). This does not mean the
3200-world calibration has been run, and it does not authorize B2-06, 2025,
or 2026.
