...
| D-007 | HIGH | CLOSED | Historical/as-of instrument metadata/tick size correctness | H2c as-of `exchange_instrument_history` model implemented and MERGED: PR #59 (`hardening/v2-h2c-asof-instrument-metadata`), merge commit `1eef3dc7d19ccb7dd9c9410f8c3b7da2b84f3572`. `storage/stage2_schema.sql`, `storage/v2_setup_readers.py::read_v2_instrument`, `storage/db.py::Database.upsert_exchange_instrument`/`fetch_v2_instrument`/`seed_current_instrument_history`/`bootstrap_instrument_metadata_revision`, `analytics/forecasting_v2/ports.py`, with pure-reader, schema, and real-PostgreSQL as-of/overlap/restart/concurrency tests, five tech-lead/CodeRabbit review rounds resolved. Closure per §I requires the implementation and tests to be MERGED, not draft — satisfied. | G0 |
| D-008 | HIGH | CLOSED | Stage-2 corrections can produce incoherent historical views | H2e publication-completeness/coherent-view/replay determinism — implemented and MERGED: PR #60 (`hardening/v2-h2e-correction-publication-coherent-replay`), merge commit `8cdd54649e1ce099511cd105badeb2ff7bdc12a6`. Closure per §I requires the implementation and tests to be MERGED, not draft — satisfied. Amended per tech-lead review round 2 (six blockers): the original `(symbol, market_type)`-only CLEAN/DIRTY status was REPLACED by a revision-comparison model, `storage/stage2_publication_state.py` — `stage2_raw_revision(symbol, market_type)`, ONE monotonic BIGINT bumped by every invalidating raw write (a correction, `xmax <> 0`, OR a first-ever insert landing inside an ALREADY-PUBLISHED derived bucket — a first insert is not automatically harmless), and `stage2_publication_state(symbol, market_type, calculation_version)` — CLEAN iff `published_raw_revision` equals the CURRENT `raw_revision`, computed by comparison, never a stored boolean; a SUPERSEDED `calculation_version` therefore correctly stays STALE even after the ACTIVE version is repaired (the original design's real gap). `Database.publish_stage2_correction` is now a compare-and-swap (`expected_raw_revision`, `SELECT ... FOR UPDATE` inside the transaction) that rejects a stale publisher atomically, mandates `exchange_feature_vectors`/`consensus_feature_vectors`, binds every row's `symbol`/`market_type`/`calculation_version` to the declared scope BEFORE any connection is acquired, and requires a real DB-provable proof (zero existing rows for that scope+version) — not merely a caller string — before accepting an empty percentile/health family. `Database.open_v2_coherent_read_session` now takes `calculation_version` and binds the returned `V2CoherentReadSession` to its exact `(symbol, market_type, calculation_version)` identity, rejecting any read call for a different identity before issuing SQL. Raw writers gained arity validation and in-batch duplicate-key last-write-wins normalization (the old `executemany` shape never needed either). `stage2_publication_state`/`stage2_raw_revision` are NEVER auto-bootstrapped CLEAN for any scope — fresh and legacy pre-H2e databases alike stay fail-closed until an explicit, real publish happens. Proven with the original six real-PostgreSQL concurrency vectors PLUS finding-specific additions: overlapping-correction stale-publisher CAS rejection, late-first-insert invalidation, active-vs-superseded-calculation_version independence, session-identity-binding rejection (before SQL), and legacy/deploy-order bootstrap fail-closed vectors, plus a deterministic replay harness (minimal-absence AND non-trivial-present-data variants, `load_v2_aligned_inputs`/`build_v2_context_snapshot`/`load_compression_breakout_inputs`). Explicitly NOT built this round (documented, not silently deferred): the `stage2_recompute_queue`-driven reconciliation WORKER (confirmed schema-only/unexecuted anywhere in this repo) and real percentile-invalidation orchestration — a scope that becomes stale today stays stale until a future worker or manual operator calls `publish_stage2_correction` with the current revision. Amended AGAIN per CodeRabbit review (round 3): `_fresh_rows_affect_published_bucket`'s per-timeframe interval CASE now fails closed for an unrecognized `exchange_feature_vectors.timeframe` (previously `ELSE interval '0'` silently excluded any such row from invalidation — fixed to unconditionally match instead, since the timeframe set is not structurally enforced anywhere by DB CHECK or dataclass validation); a real-PostgreSQL test now proves `SELECT ... FOR UPDATE` genuinely serializes a concurrent publisher and corrector on the same `stage2_raw_revision` row (task/event-based bounded-timeout non-completion proof, not sleep alone); `runtime/shadow_cli.py`'s bootstrap docstring corrected to no longer claim a `stage2_publication_state` CLEAN bootstrap that does not exist; `bootstrap_raw_revision` added to `storage/stage2_publication_state.py`'s `__all__`. | G0/G1 |
...
| C-002 | HIGH | IN_PROGRESS | Event persistence/idempotency may duplicate or partially persist same-T decisions | H3 implemented on `hardening/v2-h3-deterministic-atomic-persistence` (PR open, NOT YET MERGED — per §I closure requires MERGED, not draft): deterministic `episode_id`/`event_id` (`analytics/forecasting_v2/episode_identity.py`, SHA-256 full 64-hex-char digest of exactly the §2.1a-frozen field tuple, canonical JSON via `common.versioning.canonical_json`); `decision_code_version` added to `V2EventProvenance`/`V2EpisodeEvent`/`v2_episode_events` (closing the confirmed gap where §3.2's provenance tuple could not reach the persisted event); `Database.insert_v2_episode_events` now wraps its whole batch in one `conn.transaction()` (previously per-row autocommit) and, on an `ON CONFLICT DO NOTHING` hit, re-reads the existing row and compares it via `storage.v2_serialization.rows_semantically_equal()` before treating it as an idempotent retry — a genuine content mismatch raises `V2EventIdentityConflictError` and rolls the whole batch back, never silently accepted; a new `ux_v2ee_episode_decision_boundary` UNIQUE index enforces `(run_kind, run_id, episode_id, decision_boundary)` physically, additional to the existing PK; `assert_homogeneous_batch_scope()` rejects a batch spanning more than one `(run_kind, run_id, decision_boundary)` before any connection is acquired. Proven with pure identity/canonicalization tests (`tests/analytics/test_forecasting_v2_episode_identity.py`) and a real-PostgreSQL atomicity/idempotency/concurrency/restart suite (`tests/storage/test_v2_episode_event_transactions.py`): deterministic-retry idempotency, conflicting-retry fail-closed, concurrent identical/conflicting writers, mid-batch rollback leaving zero partial rows, LIVE/REPLAY and two-REPLAY-run isolation, one-semantic-field-difference independent identity, and a fresh-`Database`-instance restart proof. Closure per §I requires MERGED implementation + tests, not draft. | G0 |
