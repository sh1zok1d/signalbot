...
Synthetic data are never market evidence. The calibration remains a mandatory methodology gate before B2-06 scientific execution, but it cannot authorize B2-06. `B2-06_LEVERAGE_CROWDING` remains independently blocked by `FUNDING_PUBLICATION_LATENCY_UNPROVEN` and closed research/outcome/evaluator authorization.
...
`MODEL_DETECTED` is exactly:

```text
primary_positive AND bootstrap_positive AND placebo_separation
```

`STRICT_PASS_EX_MATERIALITY` is exactly:

```text
MODEL_DETECTED AND era_stability AND support_sanity
```

`STRICT_PASS` is exactly:
...
Support is therefore sparse and clustered. Mechanism-transition uniforms, `U1`, `U2`, and noise innovations are mutually independent.
...
`world_index` is zero-based. Seed derivation is SHA-256 over UTF-8 `root_seed_decimal + "|" + identity`, first 8 raw digest bytes, big-endian unsigned integer, fed to PCG64. Python `hash()` and rerolls are forbidden. DGP, bootstrap, placebo, and visibility use separate namespaces.
...
Truth-aware reporting additionally requires realized support coverage, support-conditional mean AE improvement, off-support mean AE improvement, support-run lengths, support-cluster count, raw positive support count, and an effective-support-N design-effect diagnostic from trigger autocorrelation. These truth-aware quantities never enter discovery selection.
...
Use 500 contiguous-50-row block bootstrap replicates without crossing eras.
...
Bootstrap: 50 contiguous rows/block, no cross-era block, 500 replicates, pooled mean AE improvement, q025 linear interpolation, no reroll.

Placebo: 999 replicates per candidate/world. Candidate labels are permuted separately within each scored era while Y/X1/X2/order stay fixed; refit the identical expanding-era procedure. Statistic is pooled mean AE improvement; q95 uses linear interpolation. If any nominal replicate needed for the candidate-level q95 is invalid/nonfinite, placebo separation is false.
...
Every aggregate rate receives a Wilson score 95% interval.

- specificity passes only when Wilson upper <= maximum;
- power passes only when Wilson lower >= minimum;
- power fails only when Wilson upper < minimum;
- threshold straddle = `INDETERMINATE`.
...
Materiality is independently diagnosed from `STRICT_PASS_EX_MATERIALITY` versus `STRICT_PASS` and the fraction-of-attainable metric. Full STRICT_PASS is not the sole success variable.
