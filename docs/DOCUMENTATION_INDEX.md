...
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V1_PRODUCTION_ARM.md` | **PRODUCTION_MONTE_CARLO_ARM_AUTHORIZED_UNEXECUTED** | Docs-only ARM immediate child of performance/execution freeze `ccaffe135`; authorizes one synthetic 3200-world run against reviewed implementation `9c573df`; does not execute |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V1_PRODUCTION_ARM.json` | **PRODUCTION_MONTE_CARLO_ARM_AUTHORIZED_UNEXECUTED / SUPERSEDED_FOR_REPAIRED_PRODUCTION_PY** | ARM of freeze `ccaffe135`; runtime now keys the performance freeze path; ARM `120ac45` does not authorize a descendant that changes `production.py` |
