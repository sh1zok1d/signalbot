...
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_EXECUTION_FREEZE.md` | **FROZEN_BEFORE_V2_PRODUCTION / CURRENT EXECUTABLE RUNTIME** | Docs freeze of reviewed, authority-self-reference-repaired runtime HEAD `baf9f23` / tree `bd7aee5d`; verdict `GO_FOR_NEW_IMPLEMENTATION_FREEZE`; both historical freezes remain valid; committed at the stable path/schema the repaired runtime always looks for; not an ARM |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_EXECUTION_FREEZE.json` | **FROZEN_BEFORE_V2_PRODUCTION / CURRENT EXECUTABLE RUNTIME** | Machine-readable execution freeze; binds `baf9f23` / `bd7aee5d`, current `production.py` blobs, unchanged inherited ladder, A001/A003/A004, rejected A002, rank-policy freeze, V1 TCB, canonical plan `7fa12fd3…`; a future ARM can bind this freeze with no further `production.py` edit; does not arm |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_RANK_DEGENERACY_POLICY_PRODUCTION_ARM.md` | **ARMED_FOR_ONE_CANONICAL_V2_PRODUCTION_EXECUTION** | Docs-only ARM immediate child of the V2 execution freeze `2523b389`; authorizes one canonical 3200-world V2 production calibration against reviewed implementation `baf9f23`; does not execute |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_RANK_DEGENERACY_POLICY_PRODUCTION_ARM.json` | **ARMED_FOR_ONE_CANONICAL_V2_PRODUCTION_EXECUTION** | Live ARM of freeze `2523b389`; canonical plan `7fa12fd3…`, world count 3200, A002 rejected/non-governing; authorization_consumed = false |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_RANK_DEGENERACY_POLICY_WORLD_RECORDS.json` | **CANONICAL V2 WORLD_RECORDS / IMMUTABLE** | Runtime-minted 3200-world evidence at reservation `3bbdf86`; SHA256 `d372eb00…`; not a RESULT |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_RANK_DEGENERACY_POLICY_VISIBILITY.md` | **AUTHENTICATED_CANONICAL_VISIBILITY_EVIDENCE** | Plumbing to emit frozen V1 `GROUND_TRUTH_VISIBLE` for canonical world identities; independent review `GO_FOR_RESULT_MINT`; does not rerun V2 candidates |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_RANK_DEGENERACY_POLICY_VISIBILITY.json` | **AUTHENTICATED_CANONICAL_VISIBILITY_EVIDENCE** | Mechanically bound per-world `GROUND_TRUTH_VISIBLE` evidence; SHA256 `9be8dceb…`; does not modify WORLD_RECORDS |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_RANK_DEGENERACY_POLICY_RESULT.json` | **CANONICAL V2 RESULT / MINTED / IMMUTABLE** | Runtime-minted RESULT from authenticated WORLD_RECORDS + visibility; SHA256 `761cc9af…`; frozen mechanical conclusion is the artifact, not a market-edge claim; do not remint |
