...
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V3_IMPLEMENTATION_FREEZE.md` | **FROZEN_BEFORE_V3_ARM** | Docs freeze of accepted V3 implementation `70673673` / tree `e94e18cb`; binds confirmatory/RNG/prereg/V1-lib/arch-8.0.0 selector hashes and the prereg canonical 1600-world grid; not an ARM; does not execute `10000..10399` |
| `research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V3_IMPLEMENTATION_FREEZE.json` | **FROZEN_BEFORE_V3_ARM** | Machine-readable twin; `freeze_commit_head`/`tree` intentionally `UNSET_UNTIL_THIS_COMMIT`; pre-ARM binding only |
| `scripts/research/harness_synthetic_edge_calibration_v3_authority.py` | **PRE_ARM_BINDING / NOT_ARMED** | Execution-binding helper: authenticates freeze bytes, derives scientific run identity, refuses caller kwargs/path/env substitution and canonical execution; ARM remains a later one-shot unit |
