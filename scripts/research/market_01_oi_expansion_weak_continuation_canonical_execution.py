#!/usr/bin/env python3
"""One-shot canonical MARKET-01 execution.
...
evaluate_bound_market_01. Never reruns.
"""
...
ARMED_HEAD = "9e6f398e4d0a294adff317354c64fdbce91315a2"
ARMED_TREE = "0f3092e8d61e040f34cb60f633584764c9cb6a6e"
...
LOCK_REL = "docs/research/MARKET_01_OI_EXPANSION_WEAK_CONTINUATION_CONSUMPTION_LOCK.json"
RESULT_REL = "docs/research/MARKET_01_OI_EXPANSION_WEAK_CONTINUATION_RESULT.json"
FAILURE_REL = "docs/research/MARKET_01_OI_EXPANSION_WEAK_CONTINUATION_EXECUTION_FAILURE.json"
...
def require_armed_head() -> None:
    ...
    if head != ARMED_HEAD or tree != ARMED_TREE:
        raise PreConsumptionBlocker(
            f"MARKET_01_ARM_HEAD_MISMATCH:{head}/{tree}"
        )


def authenticate_pre_execution() -> dict[str, Any]:
    require_armed_head()
    bound = authenticate_market_01_canonical_execution()
    reservation = bound["reservation"]
    if int(reservation["CANONICAL_EXECUTIONS_AUTHORIZED"]) != 1:
        raise PreConsumptionBlocker("MARKET_01_RESERVATION_AUTHORIZED_MISMATCH")
    if int(reservation["CANONICAL_EXECUTIONS_CONSUMED"]) != 0:
        raise PreConsumptionBlocker("MARKET_01_RESERVATION_ALREADY_CONSUMED")
    if MARKET_01_TEST_CALIBRATED is not False:
        raise PreConsumptionBlocker("MARKET_01_TEST_CALIBRATED_MUTATED")
    if B2_06_EXECUTION_AUTHORIZED is not False:
        raise PreConsumptionBlocker("MARKET_01_B2_06_MUTATED")
    if DEFAULT_V4 is not False:
        raise PreConsumptionBlocker("MARKET_01_DEFAULT_V4_MUTATED")
    if PROTECTED_OOS_AUTHORIZED is not False:
        raise PreConsumptionBlocker("MARKET_01_PROTECTED_OOS_MUTATED")
    if bound["run_identity"] != FROZEN_MARKET_01_RUN_IDENTITY:
        raise PreConsumptionBlocker("MARKET_01_RUN_IDENTITY_MISMATCH")
    lock_path = REPO / LOCK_REL
    if lock_path.exists():
        raise PreConsumptionBlocker("MARKET_01_CONSUMPTION_LOCK_ALREADY_PRESENT")
    result_path = REPO / RESULT_REL
    if result_path.exists():
        raise PreConsumptionBlocker("MARKET_01_RESULT_ALREADY_PRESENT")
    return bound
...
def _load_core_checksums() -> dict[str, str]:
    ...
    if identity.get("dataset_id") != PRICE_DATASET_ID:
        raise PreConsumptionBlocker("MARKET_01_CORE_SNAPSHOT_DOC_MISMATCH")
...
def load_bound_price() -> tuple[PriceView, dict[str, Any]]:
    if not CORE_ROOT.is_dir():
        raise PreConsumptionBlocker("MARKET_01_CORE_DATASET_MISSING")
    checksums = _load_core_checksums()
    try:
        paths = list_development_parquet_paths(CORE_ROOT)
    except Exception as exc:
        raise PreConsumptionBlocker(f"MARKET_01_CORE_PATHS:{exc}") from exc
    if not paths:
        raise PreConsumptionBlocker("MARKET_01_CORE_NO_DEVELOPMENT_PARQUET")
    authenticated: list[dict[str, str]] = []
    for path in paths:
        rel = f"canonical/1m/monthly/{path.name}"
        expected = checksums.get(rel)
        if expected is None:
            raise PreConsumptionBlocker(f"MARKET_01_CORE_CHECKSUM_UNBOUND:{rel}")
        digest = sha256_file(path)
        if digest != expected:
            raise PreConsumptionBlocker(f"MARKET_01_CORE_CHECKSUM_MISMATCH:{rel}")
        authenticated.append({"path": rel, "sha256": digest})
        if path.name.startswith(("2025-", "2026-")):
            raise PreConsumptionBlocker(f"MARKET_01_CORE_OPENED_FORBIDDEN:{rel}")
    ...
    for path in paths:
        ...
        if "close_time_ms" in table.column_names:
            raise PreConsumptionBlocker("MARKET_01_CORE_CLOSE_TIME_LOADED")
    ...
    if int(open_ms[-1]) >= 1_735_689_600_000:
        raise PreConsumptionBlocker("MARKET_01_CORE_REACHED_2025")
    if np.any(avail_ms != open_ms + 60_000):
        raise PreConsumptionBlocker("MARKET_01_CORE_AVAILABLE_AT_MISMATCH")
    view = PriceView(
...
def load_bound_oi() -> tuple[OiView, dict[str, Any]]:
    ...
    if not oi_path.is_file():
        raise PreConsumptionBlocker("MARKET_01_OI_JSONL_MISSING")
    if not manifest_path.is_file():
        raise PreConsumptionBlocker("MARKET_01_OI_SNAPSHOT_MANIFEST_MISSING")
    runtime = json.loads(manifest_path.read_text(encoding="utf-8"))
    runtime_id = runtime.get("snapshot_id")
    if runtime_id != OI_SNAPSHOT_ID:
        raise PreConsumptionBlocker(
            f"MARKET_01_OI_RUNTIME_SNAPSHOT_MISMATCH:{runtime_id}"
        )
    digest = sha256_file(oi_path)
    if digest != OI_JSONL_SHA256:
        raise PreConsumptionBlocker("MARKET_01_OI_JSONL_SHA256_MISMATCH")
    bound_doc = json.loads(OI_SNAPSHOT_DOC.read_text(encoding="utf-8"))
    if bound_doc.get("snapshot_id") != OI_SNAPSHOT_ID:
        raise PreConsumptionBlocker("MARKET_01_OI_BOUND_DOC_MISMATCH")
    creates: list[int] = []
    avails: list[int] = []
    values: list[float] = []
    with oi_path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if available != create + FIVE_MS:
                raise PreConsumptionBlocker("MARKET_01_OI_AVAILABLE_AT_RULE_MISMATCH")
            creates.append(create)
            avails.append(available)
            values.append(float(row["sum_open_interest"]))
    if len(creates) != OI_JSONL_ROWS:
        raise PreConsumptionBlocker(f"MARKET_01_OI_ROW_COUNT_MISMATCH:{len(creates)}")
...
def write_consumption_lock() -> str:
    payload = {
        "schema": "market_01_oi_expansion_weak_continuation_consumption_lock",
        "schema_version": "1.0.0",
        "research_id": RESEARCH_ID,
        "canonical_run_identity": FROZEN_MARKET_01_RUN_IDENTITY,
        ...
        "consumption_boundary": "FIRST_OUTCOME_BEARING_EVALUATE_BOUND_MARKET_01",
...
def consume_reservation_and_arm() -> dict[str, str]:
    ...
    reservation["MARKET_01_EXECUTED"] = True
    reservation["MARKET_01_OUTCOME_INSPECTED"] = True
    ...
    arm["MARKET_01_EXECUTED"] = True
    arm["MARKET_01_OUTCOME_INSPECTED"] = True
    arm["status"] = "EXECUTED"
...
def bind_canonical_result(
    ...
    *,
    ...
    return {
        "schema": "market_01_oi_expansion_weak_continuation_canonical_result",
        "schema_version": "1.0.0",
        "research_id": RESEARCH_ID,
        ...
        "beta_candidate": scientific.get("beta_candidate"),
...
def write_post_consumption_failure(exc: BaseException, lock_sha256: str | None) -> str:
    payload = {
        "schema": "market_01_oi_expansion_weak_continuation_execution_failure",
        "schema_version": "1.0.0",
        "research_id": RESEARCH_ID,
        "canonical_run_identity": FROZEN_MARKET_01_RUN_IDENTITY,
        "consumption_lock_sha256": lock_sha256,
...
def main(argv: list[str] | None = None) -> int:
    if argv:
        print("caller arguments cannot redefine MARKET-01 scientific authority", file=sys.stderr)
        return 2
    try:
    ...
    try:
        scientific = evaluate_bound_market_01(price, oi)
