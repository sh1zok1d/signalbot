...
# Install the Signalbot shadow forecast one-shot service + 5-minute timer,
# following the existing VPS deployment contract. DEFAULT behavior is
# install + validate ONLY (no enable, no start). Pass --enable-now to explicitly
# activate the timer.
#
# Usage:
#   sudo ./deploy/install_shadow_timer.sh                # install + validate only
#   sudo ./deploy/install_shadow_timer.sh --enable-now   # install + enable the timer
#
# This script installs deployment artifacts. It never runs --shadow-once, never
# touches Docker, never modifies the signalbot user/group or .env, and never
# stops or restarts the Stage 1 signalbot.service.
...
if [[ "${ENABLE_NOW}" -eq 1 ]]; then
  ...
  echo "To activate the 5-minute shadow timer later, run:"
  echo "  sudo ${SCRIPT_DIR}/install_shadow_timer.sh --enable-now"
fi
