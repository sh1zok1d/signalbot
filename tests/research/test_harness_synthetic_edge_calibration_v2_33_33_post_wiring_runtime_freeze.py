"""Freeze verification for the reviewed V2 33/33 post-wiring runtime.
...
HISTORICAL_33_33_FREEZE_MD_PATH = (
...
FREEZE_DOCUMENT_PATH = (
    "docs/research/HARNESS_SYNTHETIC_EDGE_CALIBRATION_V2_33_33_POST_WIRING_RUNTIME_FREEZE.md"
)
...
FREEZE_ALLOWED_CHANGED_PATHS = {
    FREEZE_ARTIFACT_PATH,
    FREEZE_DOCUMENT_PATH,
    "tests/research/test_harness_synthetic_edge_calibration_v2_33_33_post_wiring_runtime_freeze.py",
...
def test_freeze_artifact_does_not_circularly_self_hash():
    ...
    artifact_sha = _blob_sha256("HEAD", FREEZE_ARTIFACT_PATH)
...
def test_worktree_tamper_does_not_change_git_object_verification(tmp_path):
    ...
    try:
        ...
        reread = _freeze_artifact_at("HEAD")
        assert reread == freeze
        assert _blob_sha256("HEAD", FREEZE_ARTIFACT_PATH) == hashlib.sha256(original).hexdigest()
    finally:
...
def test_freeze_artifact_bindings_match_git_identities():
    ...
    assert freeze["current_executable_runtime_freeze"] is True
    assert freeze["historical_33_33_freeze_valid"] is True
    ...
    assert distinction["historical_33_33_freeze_was_invalid"] is False
    assert distinction["historical_33_33_freeze_binds_implementation"] == (
        HISTORICAL_33_33_IMPLEMENTATION_HEAD
    )
    assert distinction["current_executable_runtime_binds_implementation"] == (
    ...
    assert repairs[REVIEWED_IMPLEMENTATION_HEAD]["tree"] == REVIEWED_IMPLEMENTATION_TREE
    ...
    for label, head, tree in (
        ...
        ("reviewed_runtime", REVIEWED_IMPLEMENTATION_HEAD, REVIEWED_IMPLEMENTATION_TREE),
...
def test_historical_33_33_freeze_identity_preserved():
    freeze = _freeze_artifact_at("HEAD")
    historical = freeze["historical_33_33_implementation_freeze"]
    assert historical["head"] == HISTORICAL_33_33_FREEZE_HEAD
    assert historical["tree"] == HISTORICAL_33_33_FREEZE_TREE
    assert historical["reviewed_implementation_head"] == HISTORICAL_33_33_IMPLEMENTATION_HEAD
    assert historical["still_historically_valid"] is True
    assert historical["sufficient_for_current_executable_runtime"] is False
    artifact = historical["freeze_artifact"]
    _assert_bound_file("HEAD", artifact, identity_head=HISTORICAL_33_33_FREEZE_HEAD)
    assert artifact["sha256"] == HISTORICAL_33_33_FREEZE_ARTIFACT_SHA256
    _assert_bound_file(
        "HEAD", historical["freeze_document"], identity_head=HISTORICAL_33_33_FREEZE_HEAD
    )
    assert _git("diff", f"{HISTORICAL_33_33_FREEZE_HEAD}..HEAD", "--", HISTORICAL_33_33_FREEZE_PATH) == ""
    assert _git("diff", f"{HISTORICAL_33_33_FREEZE_HEAD}..HEAD", "--", HISTORICAL_33_33_FREEZE_MD_PATH) == ""
...
def test_governing_amendment_hashes_match_frozen_values():
    ...
    for bound in method["amendment_003"]["files"]:
        _assert_bound_file("HEAD", bound, identity_head=REVIEWED_IMPLEMENTATION_HEAD)
...
def test_canonical_v2_plan_identity_matches_freeze_artifact():
    ...
    historical = canonical_v2_plan(
        repo_root=REPO, commit=HISTORICAL_33_33_IMPLEMENTATION_HEAD
    )

    # Historical freeze/reviewed/historical implementations keep the old
    # plan identity. Live HEAD includes the repaired rank-policy bytes that
    # are part of canonical plan identity, so it must match current authority.
...
def test_freeze_production_state_not_armed_not_consumed():
    ...
    assert freeze["this_freeze_does_not"]["claim_historical_33_33_freeze_invalid"] is True
