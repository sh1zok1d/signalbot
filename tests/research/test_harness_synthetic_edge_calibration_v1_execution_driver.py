...
from tests.research.test_harness_synthetic_edge_calibration_v1_production import (
    ...
    SUPERSEDED_HISTORICAL_ARM_HEAD,
    _assert_clean,
    _authority_sha_map,
    _bind_prod,
...
def test_live_head_is_armed_unexecuted_and_binds_freeze_parent():
    ...
    historical["freeze_parent_tree"] = historical["authorized_execution_tree"]
    historical["reviewed_implementation_head"] = "d5277c42141a26948b195afe3d4ad30030151855"
...
def test_modified_tracked_freeze_artifact_cannot_arm(tmp_path, monkeypatch):
    ...
    _git(repo, "add", "-A")
    _git(repo, "commit", "-m", "tamper tracked freeze artifact")
    _commit_arm_authorizing_parent(repo)
    _bind_prod(monkeypatch, repo)
