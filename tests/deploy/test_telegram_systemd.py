"""Static validation of the Telegram-notifier systemd deployment artifacts.

Reads and validates repository files only — requires no root, no VPS, no Docker,
no PostgreSQL, no network, no installed system units, and sends no Telegram
message. `systemctl` is NEVER invoked; `systemd-analyze verify` is used only as
an OPTIONAL syntax check when the host utility is present (and its host-path
...
SERVICE = DEPLOY / "signalbot-telegram.service"
TIMER = DEPLOY / "signalbot-telegram.timer"
INSTALL = DEPLOY / "install_telegram_timer.sh"
CHECK = DEPLOY / "check_telegram_timer.sh"
DOC = DOCS / "TELEGRAM_NOTIFIER_DEPLOY.md"

EXPECTED_EXECSTART = (
    "/opt/signalbot/.venv/bin/python /opt/signalbot/main.py "
    "--telegram-once --telegram-json --log-level INFO"
)
...
def test_service_unit_ordering():
    ...
    # must NOT couple to the Stage 1 service or the shadow service (directives only)
    ...
    assert "signalbot-shadow" not in code             # no shadow unit coupling
...
def test_service_execstart_flags():
    ...
    assert text.count("--telegram-once") == 1
    assert text.count("--telegram-json") == 1
    # no explicit caps, no test/status flags baked into the timer-driven unit
    execstart = _one(_parse_unit(text)["Service"], "ExecStart")
    for banned in ("--telegram-status", "--telegram-test", "--telegram-max-scan",
                   "--telegram-max-send", "--shadow", "redis"):
...
def test_service_journal_output_and_identifier():
    ...
    assert _one(svc, "SyslogIdentifier") == "signalbot-telegram"
...
def test_timer_oncalendar_exact():
    ...
    assert oncal == "*-*-* *:00/5:35 UTC"
    assert oncal.endswith("UTC")               # UTC
    assert ":00/5:" in oncal                   # five-minute step
    assert oncal.endswith(":35 UTC")           # second :35 (offset from shadow's :05)
...
def test_timer_alignment_directives():
    ...
    assert _one(timer, "Unit") == "signalbot-telegram.service"
...
def test_installer_fixed_paths():
    ...
    assert "SERVICE_NAME=signalbot-telegram.service" in text
    assert "TIMER_NAME=signalbot-telegram.timer" in text
...
def test_installer_enables_timer_not_service():
    ...
    assert "enable --now signalbot-telegram.service" not in text
...
def test_installer_no_dangerous_operations():
    ...
    for banned in ("telegram-once", "telegram-test", "docker", "usermod", "useradd",
                   "groupmod", "groupadd", "chmod", "chown", "systemctl restart",
                   "systemctl stop", "psql", "migrate"):
        assert banned not in code, banned
    # never touches the Stage 1 or shadow units
    assert "signalbot.service" not in code
    assert "signalbot-shadow" not in code
...
def test_check_script_is_read_only_and_no_secrets():
    ...
    for banned in ("docker", "telegram-once", "telegram-test", "psql", "SELECT",
                   "INSERT", ".env", "printenv", "rm ", "cat "):
        assert banned not in code, banned
...
def test_doc_covers_required_sections():
    ...
    # manual status + optional connectivity test before activation
    assert "--telegram-status" in text and "--telegram-test" in text
    # install-only and explicit enable commands
    assert "install_telegram_timer.sh" in text
    assert "install_telegram_timer.sh --enable-now" in text
    # bootstrap / no-history behavior
    assert "bootstrapped_no_history" in low
    assert "no historical-message spam" in low or "no backlog of old predictions" in low
    # outbox durability + at-least-once + rare duplicate window
    assert "at-least-once" in low
    assert "rare" in low and "duplicate" in low
    # retry / backoff
    assert "backoff" in low and "retryafter" in low
    # advisory lock independence
    assert "separate" in low and "lock" in low
    # journal + rollback
    assert "journalctl -u signalbot-telegram.service" in text
    assert "systemctl disable --now signalbot-telegram.timer" in text
    # stage2 master switch untouched
    assert "stage2.enabled" in low and "false" in low
    # notifier independence + exclusions
    assert "no trading" in low
    assert "no outcome notifications" in low
...
def test_doc_rollback_preserves_state():
    text = DOC.read_text()
    low = text.lower()
    assert "signalbot.service" in text and "signalbot-shadow" in text  # both stay running
    assert "not** deleted" in low or "not deleted" in low
    assert ".env" in text
    # delivery tables survive rollback
    assert "telegram_notifier_state" in text
    assert "telegram_notification_deliveries" in text
    assert "preserved" in low
...
def test_doc_ff_only_update_runbook():
    ...
    assert "reset --hard" in low and "never" in low
...
def test_doc_readonly_postrun_verification_command():
    text = DOC.read_text()
    low = text.lower()
    assert "--telegram-status --telegram-json" in text
    assert "python3 -m json.tool" in text
    assert "no secret" in low
    # no raw SQL statement, no DSN literal
    assert "SELECT " not in text
    assert "postgresql://" not in low


def test_doc_no_secrets_leaked_in_doc_itself():
    text = DOC.read_text()
    # the doc explains env var NAMES but never a literal token/DSN value
...
# ============================================================================
# I. cross-isolation from the shadow deployment artifacts
# ============================================================================
def test_no_shared_unit_names_with_shadow():
    shadow_service = DEPLOY / "signalbot-shadow.service"
    shadow_timer = DEPLOY / "signalbot-shadow.timer"
    assert SERVICE.name != shadow_service.name
    assert TIMER.name != shadow_timer.name


def test_telegram_timer_offset_from_shadow_timer():
    shadow_timer = DEPLOY / "signalbot-shadow.timer"
    shadow_oncal = _one(_parse_unit(shadow_timer.read_text())["Timer"], "OnCalendar")
    telegram_oncal = _one(_parse_unit(TIMER.read_text())["Timer"], "OnCalendar")
    assert shadow_oncal != telegram_oncal
    assert shadow_oncal.endswith(":05 UTC")
    assert telegram_oncal.endswith(":35 UTC")
