...
The ONLY async/impure layer V2-H2b adds. Everything it does:
  ...
  2. decide whether this call is INITIAL PROVISIONING (`state.active is
     None` -- routed to `version_switch.provision_initial_tuple()`, legal
     for both `LIVE` and `REPLAY`) or an OLD->NEW SWITCH evaluation
     (`state.active` already set -- routed to `version_switch.evaluate_
     version_switch_transition()`, `LIVE`-only) -- these are deliberately
     SEPARATE operations (tech-lead amendment round 1, finding 2), never
     conflated at this boundary either;
  3. decide WHICH of the facts the chosen pure function might need for
     this call (`V2DrainFact` only while `DRAINING`; §3.3 activation
     readiness only once drain is already known complete, OR always for
     initial provisioning, which has no drain concept to gate on) and
  ...
  4. run the pure transition;
  5. return the result for the caller to durably, atomically persist.
...
from analytics.forecasting_v2.version_switch import (
    ...
    V2SemanticTuple, V2VersionSwitchError, V2VersionSwitchState,
...
async def resolve_version_switch_transition(
    *,
    ...
    # ---- initial provisioning: state.active is None and a target is
    # requested -- SEPARATE from OLD->NEW switching (finding 2). Always
    # needs a readiness read (there is no drain phase to defer it past). --
    if state.active is None and state.pending is None and requested is not None:
        readiness = await check_activation_readiness(
            readiness_reader, symbol=symbol, market_type=market_type,
            calculation_version=requested.calculation_version, decision_boundary=T)
        return provision_initial_tuple(
            state, decision_boundary=T, tuple_=requested, candidate_ready=readiness.ready)

    # ---- OLD->NEW switching (LIVE-only). The authoritative rejection lives
    # in the pure function (evaluate_version_switch_transition) -- but a
    # REPLAY switch attempt can NEVER succeed regardless of drain_fact, so
    # this orchestration layer must not issue a speculative drain read for
    # one either (never a read that could not possibly affect the outcome).
    # drain_fact: needed only while DRAINING, or when this call would start
    # a genuinely new switch from NO_PENDING_SWITCH (state.active already set).
    will_need_drain_fact = state.run_kind == LIVE and (
        state.phase == PHASE_DRAINING or (
