...
V2-v0 constant across the currently-implemented Stage 4/5 surface
(`family_quality.py`, `regime_4h.py`, `bias_1h.py`, `setup_common.py`,
`trend_pullback.py`, `compression_breakout.py`, `confirmed_breakout.py`),
plus a deterministic fingerprint of that manifest keyed by `rules_version`
(`config/v2.yaml`). A version-participating constant changing WITHOUT a
corresponding `rules_version` bump is exactly the silent-behavior-change
class of bug this module exists to catch --
`assert_rules_manifest_matches_version()` fails closed against it.

**Scope, deliberately narrow (no large configuration framework).** Covers
only scalar/lookback/multiplier/timedelta thresholds this package's
detectors and context engines actually gate decisions on. Deliberately
EXCLUDES: wall clock, git commit SHA, hostname, process identity, any V1
(`analytics/forecasting/`) constant, and any repo file unrelated to V2
executable rules -- a docs-only or Telegram-only change therefore cannot
change the fingerprint this module computes. Also excludes `model_
confidence`'s future weighted-sum weight table (§8) and per-family
`setup_strength`/`data_confidence` FORMULA SHAPE (not yet a separate
tunable numeric constant beyond what is already covered below) -- neither
is independently executable yet beyond the per-candidate values this PR
already freezes via the constants listed here.
...
def build_rules_manifest() -> "dict[str, Any]":
    ...
    frozen V2-v0 constant across the currently-implemented Stage 4/5
    ...
    owning module; every leaf is a `bool`/`int`/`float`/`str` (a raw
    `timedelta` is converted to `total_seconds()` for deterministic JSON
    serialization -- `timedelta` itself is not JSON-serializable)."""
    return {
        "family_quality": {
...
EXPECTED_FINGERPRINTS_BY_RULES_VERSION: "dict[str, str]" = {
    "v2-rules-v0.2.0": "eadecec884b025c7b83c8e9b3e297d7f5f1c1bf3e1d76208051ea42bcfffb914",
}
